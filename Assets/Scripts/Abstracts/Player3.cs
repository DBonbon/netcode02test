...
{   
    ...
    public List<Card3> HandCards { get; set; } = new List<Card3>();
    public List<Card3> CardsPlayerCanAsk { get; private set; }
    public List<Player3> PlayerToAsk { get; private set; } = new List<Player3>();
    ...
    /*public void AddCardToHand(Card3 card)
    {
    ...
    public void AddCardToHand(GameObject cardGameObject)
    {
        ...
        {
            // Set the parent to this GameObject's transform (keeping it under the Player3 GameObject)
            ...
            // Attempt to retrieve the Card3 component from the card GameObject
            var cardComponent = cardGameObject.GetComponent<Card3>();
            if (cardComponent != null)
            {
                // Add the Card3 component to the HandCards list
                HandCards.Add(cardComponent);
                Debug.Log($"Card {cardComponent.name} added to player {PlayerName.Value}'s HandCards list.");
            }
            else
            {
                Debug.LogError($"The GameObject {cardGameObject.name} does not have a Card3 component.");
            }
        }
        else
        {
    ...
    public void UpdatePlayerToAskList(List<Player3> allPlayers)
    {
