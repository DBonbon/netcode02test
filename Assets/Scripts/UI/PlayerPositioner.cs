...
public class PlayerPositioner : NetworkBehaviour
{
    ...
    public void ParentNonPlayerObject(GameObject nonPlayerObject)
    {
        if (nonPlayerObject != null)
        {
            nonPlayerObject.transform.SetParent(transform, false);
            nonPlayerObject.transform.localPosition = Vector3.zero; // Reset local position
            Debug.Log($"Non-player object {nonPlayerObject.name} parented to player {PlayerName.Value}.");
        }
    }
}
